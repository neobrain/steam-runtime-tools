...
flatpak_run_add_gssproxy_args (FlatpakBwrap *bwrap)
{
  ...
  const char *gssproxy_sandboxed_dir = "/run/pressure-vessel/gssproxy/";
...
flatpak_run_add_ssh_args (FlatpakBwrap *bwrap)
{
  /* Not used in pressure-vessel */
#if 0
  static const char sandbox_auth_socket[] = "/run/pressure-vessel/ssh-auth";
 ...
 * use of a proxy.
 *
 * pressure-vessel-specific: there is no app_id or instance_id argument.
 */
void
